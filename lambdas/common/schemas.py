...
class Business(BaseModel):
    ...
    phone: Optional[str] = Field(None, regex=r'^[\+]?[1-9][\d\s\-\(\)\.]{7,15}$')
...
class SEOMetadata(BaseModel):
    ...
    title: str = Field(..., min_length=10, max_length=70, description="Page title (≤70 chars)")
    meta_description: str = Field(..., min_length=50, max_length=160, description="Meta description (≤160 chars)")
    h1: str = Field(..., min_length=10, max_length=70, description="Main heading (≤70 chars)")
    slug: str = Field(..., regex=r'^[a-z0-9-]+$', description="URL-friendly slug")
...
class PageContent(BaseModel):
    ...
    introduction: str = Field(..., min_length=100, max_length=500, description="Page introduction")
    main_content: str = Field(..., min_length=800, max_length=3000, description="Main content (800+ words)")
    services_section: Optional[str] = Field(None, max_length=1000)
    location_section: Optional[str] = Field(None, max_length=800)
    conclusion: str = Field(..., min_length=100, max_length=400, description="Page conclusion")

    @validator('main_content')
    def validate_word_count(cls, v):
        word_count = len(v.split())
        if word_count < 200:
            raise ValueError(f"Main content must have at least 200 words, got {word_count}")
        return v
